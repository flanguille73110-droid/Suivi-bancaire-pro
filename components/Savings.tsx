  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Objectifs d'Épargne 🎯</h2>
        <button onClick={handleOpenAdd} className="px-6 py-3 bg-gradient-to-r from-blue-500 to-pink-500 text-white rounded-2xl shadow-xl font-bold hover:scale-[1.02] active:scale-95 transition-all">Nouvel Objectif</button>
      </div>
          ...
          return (
            <div key={goal.id} className={`group relative p-8 rounded-[2.5rem] border border-slate-100 shadow-xl transition-all ${goal.isReached ? 'bg-emerald-50 border-emerald-100 shadow-emerald-100' : goal.isPaused ? 'bg-slate-50 opacity-80 shadow-none' : 'bg-white'}`}>
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <h3 className="text-xl font-black text-slate-800 tracking-tight">{goal.name}</h3>
      ...
      {showForm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 flex flex-col">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight">{showForm === 'add' ? 'Nouveau Défi' : 'Paramètres du Projet'}</h3>
            ...
            <form onSubmit={handleSave} className="space-y-6">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Nom de l'objectif</label>
                <input type="text" required className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none font-bold focus:border-blue-500 transition-all" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="ex: Voyage au Japon 🇯🇵" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Compte Lié</label>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Déjà Économisé (€)</label>
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Date d'échéance souhaitée</label>
                <input type="date" className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent rounded-2xl outline-none font-bold focus:border-blue-500 transition-all" value={formData.deadline} onChange={e => setFormData({...formData, deadline: e.target.value})} />
              </div>
